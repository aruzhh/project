...
def monyh_link():
    ...
    with open(path + "weather.csv", 'w+', newline='' ) as file:
      column_names=['City','Date','Day temperature','Night temperature'] 
      wr = csv.DictWriter(file,fieldnames=column_names)
      wr.writerow({'City':'City','Date':'Date','Day temperature':'Day temperature','Night temperature':'Night temperature'}) 
      for x in range (len(weather_day)):
        wr.writerow({'City':cities[x],'Date' :date[x], 'Day temperature': weather_day[x],'Night temperature':weather_night[x]})
